...
__device__  double legendre_vector_basis(double x, double y, int n, int dim){
  double div_b_basis;
  x=min(max(x,-1.0),1.0);
  y=min(max(y,-1.0),1.0);
  ...
  if (dim == 0){
    switch (n){
      case 0:
         ...
         break;
      case 1:
         div_b_basis = x*sqrt_mod[1];
         break;
      case 2:
         div_b_basis = y*sqrt_mod[1];
         break;
      case 3:
         div_b_basis = x*y*sqrt_mod[1]*sqrt_mod[1];
         break;
    ...
    switch (n){
      ...
      case 5:
         ...
         break;
      case 6:
         div_b_basis= y*sqrt_mod[1];//-sqrt(3.)/4.*(x*(3.0*sqrt(5.)*y*y-sqrt(5.)-2.0));
         break;
      case 7:
         div_b_basis = x*y*sqrt_mod[1]*sqrt_mod[1];
         break;
...
__device__  double basis_ldf_t(double x, double y, int mx, int k, int var){
  ...
  case 0: //
    ...
    //basis = legendre_vector_basis(x,y,mx,0);
    break;
  case 1: //
    basis = ldf_div_basis(x,y,mx,1);
    //basis = legendre_vector_basis(x,y,mx,1);
    //basis = div_x(x,n,sq,dim);//*sqrt(2./dxx);
    break;
  }
  return basis;
...
__device__  double basis_ldf_prime(double x, double y, int mx, int my, int k, int deriv, int var){
  double basis;
  x=min(max(x,-1.0),1.0);
  y=min(max(y,-1.0),1.0);
  switch (var) {
  case 2: case 3:
    ...
    break;
  case 0: //
    //basis = ldf_div_basis_prime(x,y,mx+my*k,deriv,0);
    basis = legendre_deriv_vector_basis(x,y,mx+my*k,deriv,0);
    break;
  case 1: //
    //basis = ldf_div_basis_prime(x,y,mx+k*my,deriv,1);
    basis = legendre_deriv_vector_basis(x,y,mx+k*my,deriv,1);
    //basis = div_x(x,n,sq,dim);//*sqrt(2./dxx);
    break;
  }
  return basis;
...
__device__  double basis_prime(double x, int n, int sq, int var, int dim){
  double basis;
  x=min(max(x,-1.0),1.0);
  switch (var) {
  case 0: case 1: case 2: case 3: case 6: case 7: case 8:
    basis = legendre_prime(x,n,sq);
    break;
  case 4: //
    //basis = div_y_prime(x,n,sq,dim);//*sqrt(2./dyy);
    basis = legendre_prime(x,n,sq);
    break;
  case 5: //
    basis = legendre_prime(x,n,sq);
    //basis = div_x_prime(x,n,sq,dim);//*sqrt(2./dxx);
    break;
  }
  return basis;
}
...
__global__ void get_modes_from_nodes_ldf_test(double* nodes, double* modes, int m, int ny, int nx, int nvar){

  int id, ic, jc, im, jm, var, mid;
  int iq, jq, cid;
  int a = nx;
  int b = ny*a;
  int c = m*b;
  ...
  if( id < size ){
    ...
    for( iq=0; iq < m; iq++){
       for( jq=0; jq < m; jq++){
	       val += 0.25*(nodes[iq*b+jq*c+cid]*basis_ldf_t(xquad[iq],yquad[jq],mid,m,0) //legendre(xquad[iq],im,1)*legendre(yquad[jq],jm,1)
	              + nodes[iq*b+jq*c+cid + d]*basis_ldf_t(xquad[iq],yquad[jq],mid,m,1)) //legendre(xquad[iq],im,1)*legendre(yquad[jq],jm,1)
    	          * wxquad[iq]*wyquad[jq];
           }
    }
    modes[id] = val;
...
__global__ void get_nodes_from_modes_ldf_test(double* modes, double* nodes, int m, int ny, int nx, int nvar){
...
__global__ void compute_primitive(double* u, double* w, double gamma, int usize, int size){
  int id;
  double rho,vx,vy,vz,bx,by,bz,rho_l;
  id = blockDim.x * blockIdx.x + threadIdx.x;
  if( id < size ){
    w[id] = u[id];
...
__global__ void compute_flux(double* u, double* w, double* flux1, double* flux2, int size){
  int id;
  double rhovx,rhovy, rhovz, e,vx,vy,vz,p, bx, by, bz, bnorm, psi;
...
__global__ void compute_flux_b(double* u, double* w, double* flux1, double* flux2, double vmax, int size){
  int id;
  double rhovx,rhovy, rhovz, e,vx,vy,vz,p, bx, by, bz, bnorm, psi;
  double b1, b2, b3, v1, v2, v3;
  double ch;
...
__global__ void flux_vol_ldf (double* f_vol, double* F, double* G, double invdx, double invdy, int m, int ny, int nx, int nvar){
  int id, ic, jc, iq, jq, va;
  int im, jm, cid;
  int a = nx;
  int b = ny*a;
  ...
  if( id < size ){
    for( iq=0; iq < m; iq++){
      for( jq=0; jq < m; jq++){
      	f1 += 0.0;//f_q1[iq*b+jq*c+cid]*basis_ldf_prime(xquad[iq],yquad[jq],im,jm,m,0,0);
        g1 += G[iq*b+jq*c+cid]*basis_ldf_prime(xquad[iq],yquad[jq],im,jm,m,1,0);

        f2 += F[iq*b+jq*c+cid+d]*basis_ldf_prime(xquad[iq],yquad[jq],im,jm,m,0,1);
        g2 += 0.0;//f_q2[iq*b+jq*c+cid+d]*basis_ldf_prime(xquad[iq],yquad[jq],im,jm,m,1,1);


      }
    }
    f_vol[id] = f2*invdx + g1*invdy;// + f1*invdx + g2*invdy;//
    f_vol[id+d] = 0.0;
    f_vol[id+2*d] = 0.0;
    f_vol[id+3*d] = 0.0;
  }
...
__global__ void compute_faces_ldf(double* ufaces, double* modesu, int m, int ny, int nx, int nvar){
  int id, va, ic, jc, im, jm, q, cid, lid, rid, bid, tid;
  ...
  if( id < size ){
    ...
    for (im=0;im<m;im++){
      for (jm=0;jm<m;jm++){
	       du = modesu[im*b+jm*c+cid];
        	for (q=0;q<m;q++){
        	  shudl1[q] += du*basis_ldf(chsi_m,yquad[q],im,jm,m,0);//legendre(chsi_m,im,1)*legendre(yquad[q],jm,1);
        	  shudr1[q] += du*basis_ldf(chsi_p,yquad[q],im,jm,m,0);//legendre(chsi_p,im,1)*legendre(yquad[q],jm,1);
        	  shudb1[q] += du*basis_ldf(xquad[q],chsi_m,im,jm,m,0);//legendre(chsi_m,jm,1)*legendre(xquad[q],im,1);
        	  shudt1[q] += du*basis_ldf(xquad[q],chsi_p,im,jm,m,0);//legendre(chsi_p,jm,1)*legendre(xquad[q],im,1);

            shudl2[q] += du*basis_ldf(chsi_m,yquad[q],im,jm,m,1);//legendre(chsi_m,im,1)*legendre(yquad[q],jm,1);
        	  shudr2[q] += du*basis_ldf(chsi_p,yquad[q],im,jm,m,1);//legendre(chsi_p,im,1)*legendre(yquad[q],jm,1);
        	  shudb2[q] += du*basis_ldf(xquad[q],chsi_m,im,jm,m,1);//legendre(chsi_m,jm,1)*legendre(xquad[q],im,1);
        	  shudt2[q] += du*basis_ldf(xquad[q],chsi_p,im,jm,m,1);//legendre(chsi_p,jm,1)*legendre(xquad[q],im,1);
	             }
          }
      }
    for (q=0;q<m;q++){
...
__global__ void flux_line_integral_ldf(double* edge,double* F,double* G,int m,int ny,int nx,int nvar){
  int id, ic, jc, im, jm, va;
  int q, idF, idG, idF2, idG2;
  int a = nx;
  int b = ny*a;
  int c = m*b;
  int d = m*c;
  ...
  if( id < size){
    for(q = 0; q < m; q++){
      //valx1 += (F[q*b+idF+ 1]*legendre(chsi_p,im,1)-F[q*b+idF]*legendre(chsi_m,im,1))*legendre(yquad[q],jm,1)*wyquad[q];
      //basis_ldf(xquad[iq],yquad[jq],im,jm,m,var)
      valx1 += (F[q*b+idF+ 1]*basis_ldf(chsi_p,yquad[q],im,jm,m,0)-F[q*b+idF]*basis_ldf(chsi_m,yquad[q],im,jm,m,0))*wyquad[q];
      valy1 += (G[q*c+idG+nx]*basis_ldf(xquad[q],chsi_p,im,jm,m,0)-G[q*c+idG]*basis_ldf(xquad[q],chsi_m,im,jm,m,0))*wxquad[q];

      valx2 += (F[q*b+idF2 + 1]*basis_ldf(chsi_p,yquad[q],im,jm,m,1)-F[q*b+idF2]*basis_ldf(chsi_m,yquad[q],im,jm,m,1))*wyquad[q];
      valy2 += (G[q*c+idG2+nx]*basis_ldf(xquad[q],chsi_p,im,jm,m,1)-G[q*c+idG2]*basis_ldf(xquad[q],chsi_m,im,jm,m,1))*wxquad[q];

      //valx2 += (F[q*b+idF2+ 1]*legendre(chsi_p,im,1)-F[q*b+idF2]*legendre(chsi_m,im,1))*legendre(yquad[q],jm,1)*wyquad[q];
      //valy2 += (G[q*c+idG2+nx]*legendre(chsi_p,jm,1)-G[q*c+idG2]*legendre(chsi_m,jm,1))*legendre(xquad[q],im,1)*wxquad[q];
    }
    edge[id] = valx1 + valx2;
    edge[id+size] = valy2 + valy1;
    edge[id+2*size] = 0.0;
    edge[id+3*size] = 0.0;
  }
}
...
__global__ void grad_phi_planet (double* grad, double* x, double* y, double x0, double y0, double cutoff,  double eps, int size, double t){
  ...
  if( id < size ){
    ...
    grad[id] = dx/r*f;
    grad[id+size] = dy/r*f;
    //dx = x[id]-1.2*cos(1.0975*t);
    //dy = y[id]-1.2*sin(1.0975*t);
    //r = sqrt(dx*dx+dy*dy);
    //f = 1e-2*r*pow(r*r+4e-4,-1.5);
    //grad[id] += dx/r*f;
    //grad[id+size] += dy/r*f;
  }
}

__global__ void grad_phi_planet_pert (double* grad, double* x, double* y, double x0, double y0, double cutoff,  double eps, int size, double t){
  int id;
  id = blockDim.x * blockIdx.x + threadIdx.x;
  double dx, dy, r, f;
  if( id < size ){
    dx = x[id]-0.8*cos(1.3975*t);
    dy = y[id]-0.8*sin(1.3975*t);
    r = sqrt(dx*dx+dy*dy);
    f = 1e-4*r*pow(r*r+4e-4,-1.5);
    grad[id] += dx/r*f;
    grad[id+size] += dy/r*f;
  }
}
...
__global__ void suppress_update(double* dudt, int nx, int ny, int m, int nvar){
  int id, ic, jc, iq, jq, var;
  int im, jm, cid;
  int a = nx;
  int b = ny*a;
...
__global__ void compute_dudt_ldf(double* dudt, double* flux_vol, double* edges, double invdx, double invdy, int size){
  int id;
  id = blockDim.x * blockIdx.x + threadIdx.x;
  if( id < size ){
...
extern "C" void device_get_modes_from_nodes_ldf_b_(double** nodes, double** modes, double** bmodes){
  int size = nx*ny*m*m*nvar;
  int usize = nx*ny*m*m;
  get_modes_from_nodes_ldf_test<<<(2*usize+BLOCK-1)/BLOCK,BLOCK>>>(*nodes,*bmodes,m,ny,nx,nvar);
...
extern "C" void device_get_nodes_from_modes_ldf_b_(double** modes, double** bmodes, double** nodes){
  int size = nx*ny*m*m*nvar;
  int usize = nx*ny*m*m;
  get_nodes_from_modes_ldf_test<<<(usize+BLOCK-1)/BLOCK,BLOCK>>>(*bmodes,*nodes,m,ny,nx,nvar);
}
...
extern "C" void device_compute_dt_grav_ (){
  ...
  #ifdef SRC
    double delta_r = 0.1;
    double cutoff = 0.5-2*delta_r;
    double eps = 0.1;
    #ifdef PLANET
      grad_phi_planet<<<(usize+BLOCK-1)/BLOCK,BLOCK>>>(grad,x,y,0.0,0.0,cutoff,eps,usize,0.0);
    #else
      #ifdef LASRC
        //grad_phi_const<<<(2*usize+BLOCK-1)/BLOCK,BLOCK>>>(grad,2*usize);
        grad_phi_const<<<(usize+BLOCK-1)/BLOCK,BLOCK>>>(grad,usize);
      #else
        grad_phi<<<(usize+BLOCK-1)/BLOCK,BLOCK>>>(grad,x,y,0.0*boxlen_x,0.0*boxlen_y,cutoff,eps,usize);
      #endif
    #endif
...
__global__ void compute_max_v(double* w, double* vm, double gamma, double cfl, double dx, double dy, int m, int usize, int size){
  ...
  if(id < size){
    ...
    vmax = sqrt(pow((fabs(1.0)),2.)+pow((fabs(1.0)),2.));
    for (id = threadIdx.x+blockDim.x; id < size; id += blockDim.x){ //This is implemented considering only one block in the reduction launch.
      cs = gamma;
      d2 = 0.5*((w[id]*w[id] + w[id+usize]*w[id+usize]+w[id+usize*2]*w[id+usize*2])/1.0 + cs);
      csx = sqrt(d2 + sqrt(d2*d2-cs*1.0));//sqrt(gamma*max(w[id+usize*3],P0)/max(w[id],RHO0));
      csy = sqrt(d2 + sqrt(d2*d2-cs*1.0));//sqrt(gamma*max(w[id+usize*3],P0)/max(w[id],RHO0));
      v = sqrt(pow((fabs(1.0)),2.)+pow((fabs(1.0)),2.));
      vmax=max(v,vmax);
    }
...
extern "C" void device_compute_update_lldf_test_(int* Iter, int* SSP, double* DT, double* T, double *vm){
  double dt = *DT;
  double t = *T;
  int iter = *Iter;
  int RK = *SSP;
  ...
  get_nodes_from_modes_ldf_test<<<(usize+BLOCK-1)/BLOCK,BLOCK>>>(bmodes,u_d_q,m,ny,nx,nvar);
  compute_primitive<<<(usize+BLOCK-1)/BLOCK,BLOCK>>>( u_d_q, w, gmma, usize, usize);

  compute_flux<<<(usize+BLOCK-1)/BLOCK,BLOCK>>>(u_d_q, w, flux_q1, flux_q2, usize);

  flux_vol_ldf_t<<<(2*usize+BLOCK-1)/BLOCK,BLOCK>>>(flux_v,flux_q1,flux_q2,invdx,invdy,m,ny,nx,nvar);

  compute_faces_ldf_t<<<(nx*ny+BLOCK-1)/BLOCK,BLOCK>>>(ufaces,bmodes,m,ny,nx,nvar);
  ...
  flux_line_integral_ldf_t<<<(2*usize+BLOCK-1)/BLOCK,BLOCK>>>(edge,F,G,invdx,invdy,m,ny,nx,nvar);

  compute_dudt_ldf_t<<<(2*usize+BLOCK-1)/BLOCK,BLOCK>>>(dudt,flux_v,edge,invdx,invdy,nx*ny*m*m*2);
  ...
  if(RK==2){
    switch (iter){
    case 0:
      sum2<<<(2*usize+BLOCK-1)/BLOCK,BLOCK>>>(b_modes1,b_modes,dudt,dt,2*usize);
      break;
    case 1:
      sum3<<<(2*usize+BLOCK-1)/BLOCK,BLOCK>>>(b_modes,b_modes,b_modes1,dudt,0.5,0.5,0.5*dt, 2*usize);
      cudaDeviceSynchronize();
      break;
    }
  }
...
extern "C" void post_process_b_(int* Iter, int* SSP, double* DT, double* T, double *vm){
  double dt = *DT;
  double t = *T;
  int iter = *Iter;
  int RK = *SSP;
  ...
  get_nodes_from_modes<<<(tsize+BLOCK-1)/BLOCK,BLOCK>>>(modes,u_d_q,m,ny,nx,nvar);
  compute_primitive<<<(usize+BLOCK-1)/BLOCK,BLOCK>>>( u_d_q, w, gmma, usize, usize);
  compute_flux_b<<<(usize+BLOCK-1)/BLOCK,BLOCK>>>(u_d_q, w, flux_q1, flux_q2, v_max, usize);

  flux_vol<<<(tsize+BLOCK-1)/BLOCK,BLOCK>>>(flux_v,flux_q1,flux_q2,invdx,invdy,m,ny,nx,nvar);
  compute_faces<<<(nx*ny*nvar+BLOCK-1)/BLOCK,BLOCK>>>(ufaces,modes,m,ny,nx,nvar);
  compute_primitive<<<(4*nx*ny*m+BLOCK-1)/BLOCK,BLOCK>>>(ufaces,wfaces,gmma,4*nx*ny*m,4*nx*ny*m);

  compute_flux_b<<<(4*nx*ny*m+BLOCK-1)/BLOCK,BLOCK>>>(ufaces,wfaces,flux_f1,flux_f2,v_max, 4*nx*ny*m);
  compute_upwind<<<(m*ny*(nx+1)+BLOCK-1)/BLOCK,BLOCK>>>(ufaces,wfaces,flux_f1,F,gmma,m,ny,nx,nvar,0,bc,m*ny*(nx+1));
  compute_upwind<<<(m*(ny+1)*nx+BLOCK-1)/BLOCK,BLOCK>>>(ufaces,wfaces,flux_f2,G,gmma,m,ny,nx,nvar,1,bc,m*(ny+1)*nx);
...
extern "C" void parabolic_psi_(double* DT, double* T, double *vm){
  double dt = *DT;
  double t = *T;
  double v_max = *vm;
  double* modes;
...
extern "C" void gpu_allocation_ (int *Nvar, int* Nx, int* Ny, int* M, int* K, double *Bl_x, double *Bl_y, double *CFL, double *Eta, int* Bc, int* nequilibrium, double *Gamma) {
  ...
  //cudaMalloc ( &b_modes, 2*m*m*nx*ny * sizeof(double));
  //cudaMalloc ( &b_modes1, 2*m*m*nx*ny * sizeof(double));
